...
export class UserRepository {
    ...
    async getAllUsers() {
        ...
        try {
            const result = await conn.execute(`SELECT ID, USERNAME, NAME, EMAIL, PHONE, ROLE, CREATED_AT, UPDATED_AT
         FROM NQT_USER.USERS`, [], { outFormat: oracledb.OUT_FORMAT_OBJECT });
            return result.rows || [];
        }
        finally {
            await conn.close();
    ...
    async insertOrUpdateUser(data) {
        ...
        try {
            ...
            const check = await conn.execute(checkSql, { username: data.username }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
        ...
        `;
                await conn.execute(updateSql, { username: data.username, password: hashedPassword }, { autoCommit: true });
